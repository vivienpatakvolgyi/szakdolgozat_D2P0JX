...
def run_experiment():
    checkpoint = keras.callbacks.ModelCheckpoint(
        "video_classifier_mobilenet.h5", save_weights_only=True, save_best_only=True, verbose=1
    )
...
ready_model.load_weights('video_classifier_mobilenet.h5')
...
# %%
with open('Mnet_pred_1.txt', 'w') as f:
    for i in range(y_pred1.shape[0]):
        f.writelines(str(f"{i}\t{np.argmax(y_pred1[i])}\n"))

# %%
with open('Mnet_pred_2.txt', 'w') as f:
