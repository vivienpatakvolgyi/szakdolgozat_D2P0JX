...
with open('../data/train_val_test_paths_labels.pkl', 'rb') as f:
    ...
    parser.add_argument('-n', '--name',
                        default='SV-RCNeT_epoch-25_length-4_batch-56_trainN-17_valN-2_testN-2_resize-24_test_6799.pkl',
                        type=str, help='name of model')
    parser.add_argument('-a', '--test', default=2, type=int, help='number of test video(s)')
    parser.add_argument('-r', '--root', default='SV-RCNet', type=str, help='Root directory for videos')

    args, unknown = parser.parse_known_args()
    sequence_length = args.seq
    pred_name = 'out\\' + args.name
    num_test = args.test
    root_dir = args.root
    ...
    print('(num_preds + (sequence_length - 1) * 40) : {:6d}'.format(num_preds + (sequence_length - 1) * num_test))
...
if num_labels == (num_preds + (sequence_length - 1) * num_test):
    ...
    phase_dir = 'out\\' + os.path.splitext(args.name)[0] + '_prediction'
    print(phase_dir)
    ...
    for i in range(num_test):
        filename = phase_dir + '/video' + str(i + 1) + '-phase.txt'
        f = open(filename, 'a')
        f.write('Frame  Phase')
        f.write('\n')
        preds_each = []
        ...
        for k in range(len(preds_each)):
            f.write(str(1 * k))
            f.write('\t')
            f.write(str(preds_each[k].item()))  # f.write(phase_dict_key[preds_each[k]])
            f.write('\n')
        f.close()
    ...
    for i in range(len(test_labels)):
        if test_labels[i][0] == preds_all[i].item():  # if test_labels[i][len(test_num_each) - 1] == preds_all[i]:
            test_corrects += 1
