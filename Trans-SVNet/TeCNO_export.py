...
with open('./train_val_paths_labels_21.pkl', 'rb') as f:
    ...
    parser.add_argument('-n', '--name',
                        default='TeCNO50_epoch_7_train_9408_val_8883_test_6710.pkl',
                        type=str, help='name of model')
    parser.add_argument('-a', '--test', default=2, type=int, help='number of test video(s)')
    parser.add_argument('-r', '--root', default='TeCNO', type=str, help='Root directory for videos')

    args, unknown = parser.parse_known_args()
    sequence_length = args.seq
    pred_name = './best_model/TeCNO/'+input('Please enter the name of the TeCNO prediction (.pkl file)\n').split('.')[0]+'.pkl'
    num_test = args.test
    root_dir = args.root
...
if num_labels == (num_preds + (sequence_length - 1) * num_test):
    ...
    for i in range(num_test):
        filename = phase_dir + '/tecno_video' + str(i + 1) + '-phase.txt'
        f = open(filename, 'a')
        f.write('Frame  Phase')
        f.write('\n')
        preds_each = []
